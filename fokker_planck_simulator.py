...
class FokkerPlanckSimulator:
    ...
    def run_simulation(self, pure_parameter = False, compare_analytical = False):
        ...
        x_var, y_var, a, b, c, d, e, f = sp.symbols('x_var y_var a b c d e f')

        def solve_fkpk_symbol(PT_model,x, y, a, b, c, d, e, f):
            # 定義符號變數
            ...
            return PT_const, PTx, PTy, PTxx, PTyy, PTxy
        gamma_var, nu_var, n_th_var = sp.symbols('gamma_var nu_var n_th_var')

        # Use exponential function to simulate the solution of Fokker-Planck eq.
        def PT_model_QREP(x,y,Px,Py,Pxx,Pyy,Pxy):
            PT1 = gamma_var + (gamma_var / 2 * x) * Px + (gamma_var / 2 * y) * Py
            PT2 = gamma_var * (n_th_var + 1) / 4 * (Pxx + Pyy)
            return PT1 + PT2
        
        PT_const, PTx, PTy, PTxx, PTyy, PTxy = solve_fkpk_symbol(PT_model_QREP,x_var, y_var, a, b, c, d, e, f)

        a_prime = sp.lambdify((a,b,c,d,e,f,gamma_var,nu_var,n_th_var), PT_const, 'numpy')
        b_prime = sp.lambdify((a,b,c,d,e,f,gamma_var,nu_var,n_th_var), PTx, 'numpy')
        c_prime = sp.lambdify((a,b,c,d,e,f,gamma_var,nu_var,n_th_var), PTy, 'numpy')
        d_prime = sp.lambdify((a,b,c,d,e,f,gamma_var,nu_var,n_th_var), PTxx, 'numpy')
        e_prime = sp.lambdify((a,b,c,d,e,f,gamma_var,nu_var,n_th_var), PTyy, 'numpy')
        f_prime = sp.lambdify((a,b,c,d,e,f,gamma_var,nu_var,n_th_var), PTxy, 'numpy')
        ...
        with open(log_filename, 'w') as log_file:
            ...
            if pure_parameter:
                ...
                for t in tqdm(range(1, self.nsteps), desc="Simulating", unit="step"):
                    self.solution[t] = self.solver(self.t_vals[t-1], self.solution[t-1], self.dt, self.phys_parameter, time_deriv_funcs)
